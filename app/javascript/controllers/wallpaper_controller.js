...
export default class extends Controller {
  ...
  fontSize() {
    ...
    return Math.floor(i / aspect) * i < count ? this.options.height / Math.ceil(i / aspect) : this.options.width / i;
  }
  ...
  loadOptions() {
    try {
      const options = JSON.parse(atob(localStorage.getItem("options")));
      this.options = {...this.defaultOptions, ...options};
    } catch (error) {
      console.log("Failed to load options from localStorage.", error);
      this.loadDefaults();
    }
  }
  ...
  loadProgress() {
    ...
    fetch(uri).then((response) => response.json()).then((data) => {
      ...
      this.apiKey = this.apiKeyTarget.value;
  ...
  render() {
    ...
    const fontPadding = this.options.spacing;
    const rows = Math.floor(this.options.height / (fontSize - 1));
    const columns = Math.ceil(this.kanjiArray.length / rows);
    const gridSize = rows * columns;
    const xPad = (this.options.width - fontSize * columns) / columns;
    const yPad = (this.options.height - fontSize * rows) / (rows + 1);

    const fontPrefix = "900 " + (fontSize - fontPadding) + "px ";
    let fontFamily = this.options.fontFamily;
    if (fontFamily == "" || !document.fonts.check(fontPrefix + fontFamily)) {
      fontFamily = "sans-serif";
    }
    context.font = fontPrefix + fontFamily;
    ...
    for (var i = 0; i < this.kanjiArray.length; ++i) {
      ...
      context.fillStyle = this.options.levelColors[level];
      Draw(character);
    }

    const paths = this.fillImageTarget.querySelectorAll("path");
    paths.forEach((p) => p.fill = this.options.levelColors[0]);
    ...
    context.fillStyle = this.options.levelColors[0];
    Draw("丸");
    Draw("森");
  ...
  saveOptions() {
    localStorage.setItem("options", btoa(JSON.stringify(this.options)));
  }

  setInputs() {
    this.widthTarget.value = this.options.width;
    this.heightTarget.value = this.options.height;
    this.spacingTarget.value = this.options.spacing;
    this.fontFamilyTarget.value = this.options.fontFamily;
    this.sortByTarget.value = this.options.sortBy;
    for (var i = 0; i < this.options.levelColors.length; ++i) {
      this.levelColorTargets[i].value = this.options.levelColors[i];
    }
  }
  ...
  update() {
    this.options.width = this.widthTarget.value;
    this.options.height = this.heightTarget.value;
    this.options.spacing = this.spacingTarget.value;
    this.options.fontFamily = this.fontFamilyTarget.value.trim();
    this.options.sortBy = this.sortByTarget.value;

    for (var i = 0; i < this.options.levelColors.length; ++i) {
      this.options.levelColors[i] = this.levelColorTargets[i].value;
    }
    this.options.backgroundColor = this.backgroundTarget.value;

    const dpr = window.devicePixelRatio;
    if (this.options.width == 0) {
      this.options.width = Math.floor(window.screen.width * dpr);
      this.widthTarget.value = this.options.width;
    }
    if (this.options.height == 0) {
      this.options.height = Math.floor(window.screen.height * dpr);
      this.heightTarget.value = this.options.height;
    }

    this.canvasTarget.width = this.options.width;
    this.canvasTarget.height = this.options.height;
